...
func (rs *restServer) Close() error {
	errs := make([]error, 0)
	if rs.crudMap != nil {
		errs = append(errs, rs.crudMap.Close())
	}
...
func (rs *restServer) Run() (err error) {
	...
	if rs.cfg.Server.HTTPTLSConfig != nil {
		return rs.server.ListenAndServeTLS(rs.cfg.Server.HTTPTLSConfig.TLSCertPath, rs.cfg.Server.HTTPTLSConfig.TLSKeyPath)
	} else {
